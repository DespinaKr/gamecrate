...
type GiftPlan = {
    ...
    months: number;
    discountRate: number;
    popular?: boolean; // προαιρετικό
};

const giftPlans: GiftPlan[] = [
    { id: "3month", duration: "3 Months", months: 3, discountRate: 0.05 },
    { id: "6month", duration: "6 Months", months: 6, discountRate: 0.12, popular: true },
    { id: "12month", duration: "12 Months", months: 12, discountRate: 0.20 },
];


// Formatters για € (ολόκληρα ποσά) και €/μήνα (2 δεκαδικά)
...
function calcTotals(months: number, tier: Tier, discountRate: number) {
    const monthly = TIER_PRICING[tier];
    const base = months * monthly;                // άθροισμα χωρίς έκπτωση
    const discount = Math.round(base * discountRate); // στρογγυλοποίηση σε € για καθαρό “Save”
    const total = Math.max(0, base - discount);   // τελικό ποσό προπληρωμής
    const perMonth = total / months;              // αποτελεσματικό €/μήνα
    return { base, discount, total, perMonth };
}
...
const GiftCrate = () => {
    ...
    const active = giftPlans.find(p => p.id === selectedPlan) ?? giftPlans[1];
    const { total: activeTotal, perMonth: activePerMonth, discount: activeDiscount } =
        calcTotals(active.months, selectedTier, active.discountRate);
                    ...
                    <div className="max-w-5xl mx-auto">
                        ...
                        <div className="mb-12">
                            ...
                            <div className="flex justify-center gap-4">
                                {([
                                    { id: "standard", name: "Standard", price: TIER_PRICING.standard },
                                    { id: "premium", name: "Premium", price: TIER_PRICING.premium },
                                ] as const).map((tier) => (
                                    <Button
                                        key={tier.id}
                                    ...
                                    >
                                        {tier.name} ({EUR0.format(tier.price)}/mo)
                                    </Button>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="mb-12">
                            ...
                            <div className="grid md:grid-cols-3 gap-6">
                                {giftPlans.map((plan) => {
                                    const { total, discount, perMonth } = calcTotals(plan.months, selectedTier, plan.discountRate);
                                        ...
                                        >
                                            ...
                                            <CardContent className="p-6 text-center">
                                                ...
                                                <p className="text-muted-foreground text-sm mb-1">
                                                    Save {EUR0.format(discount)}
                                                </p>
                                                <div className="text-xs text-muted-foreground mb-4">
                                                    ({EUR2.format(perMonth)}/mo)
                                                </div>
                            ...
                            <div className="mt-8 pt-6 border-t border-border flex flex-col md:flex-row items-center justify-between gap-4">
                                <div>
                                    ...
                                    <p className="font-display text-3xl font-bold text-gradient-pink">
                                        {EUR0.format(activeTotal)}
                                    </p>
                                    <p className="text-sm text-muted-foreground">
                                        ({EUR2.format(activePerMonth)}/mo · Save {EUR0.format(activeDiscount)})
                                    </p>
                                </div>
